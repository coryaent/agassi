...
async function putRecord (_fqdn, _type, _record) {
    ...
    // log.debug ('existingRecordLineIndex:', )
...
async function deleteRecord (_fqdn, _type, _record) {
    ...
    let deletion = await axios.get (`https://${cPanelServer}/execute/DNS/mass_edit_zone?zone=${fqdn.domain}&serial=${serial}&remove=${existingRecord.line_index}`, auth);
    deletion = deletion.data;
    log.debug ('deletion:', deletion);
    if (deletion.errors) {
        throw new Error (deletion.errors.toString ());
    } else {
        if (deletion.warnings) {
            log.warn ('warning:', deletion.warnings.toString ());
        } else {
            log.debug (`${type} record removed`);
        }
    }
}

async function deleteCnameRecord (fqdn, record) {
    return deleteRecord (fqdn, 'CNAME', record);
}

async function deleteTxtRecord (fqdn, record) {
    return deleteRecord (fqdn, 'TXT', record);
}

async function test () {
    await putCnameRecord ('cname3.test.staging.corya.co', 'domain.invalid.');
    await putTxtRecord ('text11.test.staging.corya.co', 'myTextValue');
    await deleteCnameRecord ('cname3.test.staging.corya.co', 'domain.invalid.');
    await deleteTxtRecord ('text11.test.staging.corya.co', 'myTextValue');
}

// set a different ttl and see if the updating works
test ();
